...
interface EditableExcursion extends Excursion {
  editing?: boolean;
...
export class AdminPageComponent implements OnInit {
  ...
  constructor(
    ...
    console.log(
      '[AdminPageComponent] - constructor: Admin page component initialized',
    );
  }
  ...
  loadExcursions() {
    console.log(
      '[AdminPageComponent] - loadExcursions: Loading excursions from service',
    );
    this.loadingExcursions = true;
    this.excursionService.getExcursions().subscribe({
      next: (data) => {
        console.log(
          '[AdminPageComponent] - loadExcursions: Received excursions data:',
          data.length,
          'excursions',
        );
        ...
        console.log(
          '[AdminPageComponent] - loadExcursions: Excursions loaded successfully',
        );
      },
      error: (err) => {
        console.error(
          '[AdminPageComponent] - loadExcursions: Error loading excursions:',
          err,
        );
        this.errorExcursions =
          err?.error?.message || 'Erro ao carregar excursões';
        this.loadingExcursions = false;
      },
    });
  }
  ...
  toggleEdit(excursion: EditableExcursion) {
    console.log(
      '[AdminPageComponent] - toggleEdit: Toggling edit mode for excursion:',
      excursion.name,
    );
    excursion.editing = !excursion.editing;
    console.log(
      '[AdminPageComponent] - toggleEdit: Edit mode set to:',
      excursion.editing,
    );
    if (excursion.editing) {
      ...
      (excursion as any).returnDate = this.toDateInputValue(
  ...
  onFileSelected(event: Event, excursionId?: string) {
    ...
    if (file) {
      if (excursionId) {
        ...
        console.log(
          '[AdminPageComponent] - onFileSelected: File selected for excursion:',
          excursionId,
          file.name,
        );
      } else {
        ...
        console.log(
          '[AdminPageComponent] - onFileSelected: File selected for new excursion:',
          file.name,
        );
      }
    }
  ...
  saveExcursion(excursion: EditableExcursion) {
    console.log(
      '[AdminPageComponent] - saveExcursion: Saving excursion:',
      excursion.name,
    );
    if (!excursion._id) {
      console.error(
        '[AdminPageComponent] - saveExcursion: No excursion ID provided',
      );
      return;
    }
    ...
    this.adminService.updateExcursion(excursionId, excursion, file).subscribe({
      next: (res) => {
        console.log(
          '[AdminPageComponent] - saveExcursion: Excursion saved successfully:',
          res,
        );
      ...
      error: (err) => {
        console.error(
          '[AdminPageComponent] - saveExcursion: Error saving excursion:',
          err,
        );
  ...
  deleteExcursion(excursion: EditableExcursion) {
    console.log(
      '[AdminPageComponent] - deleteExcursion: Attempting to delete excursion:',
      excursion.name,
    );
    if (!excursion._id) {
      console.error(
        '[AdminPageComponent] - deleteExcursion: No excursion ID provided',
      );
      return;
    }
    if (!confirm('Confirmar exclusão?')) {
      console.log(
        '[AdminPageComponent] - deleteExcursion: User cancelled deletion',
      );
      return;
    }
    this.adminService.deleteExcursion(excursion._id).subscribe({
      next: () => {
        console.log(
          '[AdminPageComponent] - deleteExcursion: Excursion deleted successfully',
        );
        this.excursions = this.excursions.filter(
          (e) => e._id !== excursion._id,
        );
      },
      error: (err) => {
        console.error(
          '[AdminPageComponent] - deleteExcursion: Error deleting excursion:',
          err,
        );
  ...
  createExcursion() {
    console.log(
      '[AdminPageComponent] - createExcursion: Creating new excursion:',
      this.newExcursion,
    );
    this.createMsg = '';
    if (!this.newExcursion.name) {
      console.log(
        '[AdminPageComponent] - createExcursion: Validation failed - name is required',
      );
    ...
    this.creating = true;
    this.adminService
      .createExcursion(this.newExcursion, this.selectedFile || undefined)
      .subscribe({
        next: (created) => {
          console.log(
            '[AdminPageComponent] - createExcursion: Excursion created successfully:',
            created,
          );
        ...
        error: (err) => {
          console.error(
            '[AdminPageComponent] - createExcursion: Error creating excursion:',
            err,
          );
          this.creating = false;
  ...
  disenrollUser(ex: EditableExcursion, u: string | ExcursionUserRef) {
    ...
    this.adminService.disenrollUserFromExcursion(ex._id, uid).subscribe({
      ...
      error: (err) => {
        alert(
          err?.error?.error || err?.error?.message || 'Erro ao desinscrever',
        );
        this.enrolleeBusy.delete(key);
      },
    });
  ...
  onPaidCheckboxChange(
    ex: EditableExcursion,
    u: string | ExcursionUserRef,
    ev: Event,
  ) {
  ...
  submitChangeUserType() {
    ...
    this.adminService
      ...
      .pipe(
        switchMap((u) =>
          this.adminService.patchUserRole(String(u.id), {
